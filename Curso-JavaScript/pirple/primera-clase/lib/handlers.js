                ...
                if(tokenIsValid) {
                    ...
                    _data.read('users',phone,function(err,dataUser){
                        if(!err && dataUser){
                            ...
                            if(password){
                                dataUser.password= helpers.hash(password);
                            }
                            _data.update('users',phone,dataUser,function(err){
                                if(!err){
                                    callback(200);
                                } else{
...
handlers._users.delete= function(data,callback){
    // Check the number phone is valid
    let phone= typeof(data.queryStringObject.phone)=='string' && data.queryStringObject.phone.trim().length==9 ? data.queryStringObject.phone.trim() : false;
...
//Container fot all the tokens methods
handlers._tokens={};
...
handlers._tokens.post=function(data,callback){
    ...
    let password= typeof(data.payload.password)=="string" && data.payload.password.trim().length>0 ? data.payload.password.trim() : false;
    if(phone && password){
        // Lookup the use who matches the phone number
        _data.read('users',phone,function(err,dataUser){
            if(!err && dataUser){
...
handlers._tokens.put=function(data,callback){
    ...
    if(id && extend){
        // Lookup the token
        _data.read('tokens',id,function(err,dataToken){
            if(!err && dataToken){
                // Check to the make sure the token isn't already expired
                if(dataToken.expires>Date.now()){
                    // Send the expiration an hour from now
                    dataToken.expires= Date.now()+1000*60*60;

                    // Store the new updates
                    _data.update('tokens',id,dataToken,function(err){
                        if(!err){
                            callback(200);
                        } else{
    ...
    if(id){
        ...
        _data.read('checks',id,function(err,checkData){
            if(!err && checkData){
                ...
                //Verify the given token is valid and belongs the user object before returning it to the requester
...
handlers._checks.put= function(data,callback){
    ...
    if(id){
        ...
        if(protocol || url || method || successCodes || timeoutSeconds){
            ...
            callback(400,{'Error':'There is not any fields to update'});
        }
    } else{
        callback(400,{'Error':'Missing required fields'});
    }
                              ...
                              _data.read('users',checkData.userPhone,function(err,userData){
                                  if(!err && userData){
                                      ...
                                      if(checkPosition>-1){
                                          ...
                                          callback(400,{'Error':'Could not find the check on the users object,so could not remove it'});
                                      }
