...
// Special logic for accomodating a check that has never been tested before (don't alert on the one)
workers.processCheckOutcome= function(originalCheckData, checkOutcome){
    ...
    _data.update('checks',newCheckData.id,newCheckData,function(err){
       if(!err){
           // Sent the new check data to the next phase in process if needed
           if(alertWarranted){
...
workers.alertUserToStatusChange= function(newCheckData){
    ...
    helpers.sendTwilioSms(newCheckData.userPhone,msg,function(err){
       if(!err){
           ...
           debug('Error: Could not send the sms alert to the user who had a state change in their check');
       }
    });
};
